...
- PR #517 (`fix(opencode): stabilize retry, error and remount lifecycle`) was merged into **integration only** on 2026-09-17 after the repository owner reported that the real Zorin/OpenCode validation passed on exact feature head `bc50c65e18169d9adbf9f0e4d0e9b82b8a58cafd` and the full automated campaign was green.
...
- PR remains **draft** until the exact-head automated campaign is green;
...
User-reported problem: launcher startup output had become difficult to use because the machine/harness summary appeared twice, raw pairing deep links and alternate endpoints were printed below the QR, and child runtimes printed `0.0.0.0` plus internal metadata as though those were useful client instructions.

The intended output contract is now:

- one launcher-owned **Connection** block;
- one preferred reachable `Machine URL`, never a list of every interface;
- username/password exactly once;
- `Open in browser` only when a real browser origin is configured through CORS;
- for the hosted web client, `https://giuliastro.github.io` maps to the actual app URL `https://giuliastro.github.io/harness-remote/`, whose origin remains exactly the configured CORS origin;
- one **Harnesses** list, with the primary first and no repetitive `available` wording;
- one optional phone-pairing QR;
- no raw `harnessremote://...` URI, token, alternate interface URL or duplicate endpoint under the QR;
- if QR rendering is unavailable, fall back to the already printed manual Machine URL and credentials;
...
Behavioral coverage in #519 locks the CORS/browser-origin relationship, preferred-address selection, primary-first harness ordering, pairing-output redaction and launcher/child output ownership.

Because this status update changes the feature head, **all applicable gates must be green again on the new exact SHA before #519 is eligible for merge**.
...
The intended next release is **Harness Remote 3.1.0**. Do not create a new RC or merge development to `main` merely because #517 is integrated.

Remaining release-readiness work still includes the applicable true-boundary evidence tracked by issue #368, including real harness/runtime restart and persisted-Session recovery, physical Android foreground/background plus network interruption/reconnect, and repository-admin enforcement on `main` where the connected GitHub App lacks permission to make that administration write.
