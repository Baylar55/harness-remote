...
- PR #485 retired two redundant model-bootstrap source guards from `model-regression.test.mjs` in favor of the blocking Chromium model-switch smoke, which holds `/models` unresolved and proves the composer and Send stay gated until a verified catalog arrives. Full Chromium, desktop matrix and signed Debug APK gates passed before integration.
...
- PR #499 refreshed this handoff after #497; docs only.
...
The integration branch remains in release-candidate stabilization. Do not start another source-guard cleanup, P3 provider expansion or opportunistic runtime refactor before the next release. The intended next release is **Harness Remote 3.1.0**, not 3.0.3: compared with 3.0.2, the integration line contains substantial user-facing P1/P2 work including pairing/onboarding, Attention semantics, desktop-owned local runtime/recovery, Project/outcome evidence and cross-machine Native Session continuity.

Release publication remains blocked by true-boundary evidence and repository administration plus any evidenced release-blocking regression fix such as the current OpenCode rail correction:
...
External PR #504 (`daemon: support custom ACP primaries with multiple detected CLIs`) remains blocked and is not part of the 3.1.0 candidate. Re-review only after the intended contract is coherent end-to-end and executable coverage exercises the real parse/startup path rather than only the helper.

External PR #494 was closed without merge after real validation showed that its central Mimocode product goal was not met in the normal OpenCode + Mimocode installation case: both CLIs were detected but only OpenCode was exposed as the managed backend. Its external-session behavior was also narrower than the PR description suggested. Do not revive or port #494 as part of 3.1.
