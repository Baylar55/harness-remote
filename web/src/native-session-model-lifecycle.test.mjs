...
// --- 9. An existing native Session owns its exact model catalog ---------------------------------
sent.length = 0
responder = (url) => {
...
const sessionCatalog = await taskClient.listAgentModels(target().config, 'claude', {
...
assert.equal(freshCatalog.models[0]?.modelID, 'claude-opus-4-8')
