...
assert.ok(observer.includes('withOpenCodeLiveLifecycle(conversation, liveStatus, liveError)'), 'OpenCode retry/error lifecycle must overlay presentation without mutating durable controller state')
assert.ok(observer.includes('conversation={presentedConversation}'), 'native Session observer must pass the lifecycle-aware runtime to the shared conversation surface')
assert.ok(observer.includes('target.backend === "opencode"'), 'the live lifecycle overlay must stay scoped to OpenCode rather than changing ACP harness semantics')
