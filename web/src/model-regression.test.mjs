...
assert.match(adapter, /reconcileNativeSessionModel/, 'the native projection must absorb authoritative model metadata')
assert.match(adapter, /for \(const run of entry\.runs\.values\(\)\)/, 'recovered models must fill Runs that never recorded one')
