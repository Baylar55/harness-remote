...
assert.ok(prompt.includes('existing.text !== normalized'), 'a different prompt must not jump ahead of unresolved native delivery')
...
assert.ok(observer.includes('sendNativeSessionPrompt(target, text)'), 'continuation must send through the idempotent native Session operation path')
assert.ok(observer.includes('loadPendingNativeSessionPrompt(target)?.text'), 'a lost-response prompt must be restored into the composer after reload')
