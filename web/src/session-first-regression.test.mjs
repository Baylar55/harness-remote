...
assert.ok(routing.includes('createRemoteSessionHandoff'), 'cross-machine continuation must create its target idempotently on the target daemon')
assert.ok(routing.includes('registerNativeSessionLink'), 'cross-machine continuation must mirror lineage onto the source daemon')
...
assert.ok(workThread.includes('routing.onContinue') && workThread.includes('routeChanged'), 'the mature v3 composer must own send-time destination switching')
...
assert.ok(standalone.includes('openLinkedSession') && standalone.includes('nativeSessionTransferredContext'), 'source and target Sessions must expose navigable lineage and transferred context')
