...
assert.ok(routing.includes('PENDING_ROUTE_TTL_MS = 10 * 60 * 1000'), 'routed continuation must not leave a permanent unresolved localStorage block')
assert.ok(handoff.includes('PENDING_HANDOFF_TTL_MS = 10 * 60 * 1000'), 'resource-creating handoff retries must also have a finite recovery window')
assert.ok(routing.includes('loadPendingNativeSessionPrompt(routedTarget)') && routing.includes('clearPendingNativeSessionRoute(source)'), 'definite first-prompt rejection must release the routed continuation')
