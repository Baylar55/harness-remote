...
assert.ok(adapter.includes('api.listStatuses(entry.target.config)'), 'native projection status must use the same harness-global authority as Session Home')
assert.equal(adapter.includes('api.listStatuses(entry.target.config, entry.target.directory)'), false, 'native projection status must not block on directory-scoped OpenCode status')
