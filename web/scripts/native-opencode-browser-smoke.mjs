...
async function assertExistingContract(browser, viewport, mobile) {
  ...
  const preparing = page.locator(".uw-message-agent:not(.uw-message-pending) strong[role='status']").filter({ hasText: "OpenCode is getting started" })
  ...
  assert.equal(await preparing.locator("xpath=..").locator("time").count(), 0, "OpenCode preparation must not look like a completed timestamped response")
